...
module.exports = {
    ...
    olderRevision: (gitDirectory, filenames, age) => {
        filenames = filenames.filter(filename => typeof filename === 'string');

        const relativeFilenames = filenames.map(filename => path.relative(gitDirectory, filename));

        return helpers
            .getOutput('git', ['--no-pager', 'log', `-${age}`, '--pretty=format:%h'].concat(relativeFilenames))
            .then(revisionsStr => {
                const revisions = revisionsStr.trim().split('\n');
                if (revisions.length < age) {
                    throw new TooOldRevisionError();
                } else {
                    return Promise.all(relativeFilenames.map(filename => helpers.getOutput(
                        'git',

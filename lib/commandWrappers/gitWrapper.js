...
module.exports = {
            ...
            fs.stat(path.resolve(directory, '.git'))
                .then((error, stats) => {
                    if (error) {
                        ...
                        helpers.getOutput('git', ['rev-parse', '--git-dir'], 500)
    ...
    clone: (repo, directory) => {
        return helpers.runInherited('git', ['clone', repo, directory], {timeout: 0});
    },
    fetch: (gitDirectory) => {
        return helpers.runInherited('git', ['fetch'], {timeout: 0, cwd: gitDirectory});
    },
    checkout: (gitDirectory, gitId) => {
        return helpers.runInherited('git', ['checkout', gitId], {timeout: 0, cwd: gitDirectory});
    },
    NotAGitRepoError,

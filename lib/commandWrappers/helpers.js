...
module.exports = {
    getOutput: function getOutput(executable, args, timeout = 500) {
     ...
     */
    runInherited: function runPiped(executable, args, {timeout = 500, cwd=process.cwd()}) {
        return new Promise((resolve, reject) => {
            const commandName = `[${executable} ${args.join(' ')}]`;

            let completed = false;

            const proc = childProcess.spawn(executable, args, {stdio: 'inherit'});

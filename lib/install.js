 ...
 * @returns Promise
 */
function install({force = false, config, rePull = false}) {
    return new Promise((resolve, reject) => {
        ...
        checkNodeModules(force)
            .then(() => {
                ...
                return fsExtra.readFile(pkgJsonPath)
            })
            .then(buf => {
...
function tryOlderBundles(config, newPkgJson) {
    return new Promise((resolve, reject) => {
        ...
        return gitWrapper.isGitRepo(path.dirname(pkgJsonPath))
            .then(() => {
                return pullOlderBundle(config, pkgJsonUtils.calcHash(newPkgJson));
            })
            .then(oldPkgJson => {
...
function pushBackends(backends, hash, config, rePull) {
    const logger = getLogger();

...
const LOCKFILE_TYPES = ['npm-shrinkwrap.json', 'package-lock.json', 'yarn.lock'];
...
function tryOlderBundles(config, newPkgJson) {
    return new Promise((resolve, reject) => {
        ...
        return gitWrapper.isGitRepo(path.dirname(pkgJsonPath))
            .then(() => {
                return pullOlderBundle(config, pkgJsonUtils.calcHash(newPkgJson, null, config.packageHash));
            })
            .then(oldPkgJson => {
...
function pullOlderBundle(config, prevHash, historyIndex = 0) {
    return new Promise((resolve, reject) => {
        const logger = getLogger();
        ...
        gitWrapper.olderRevision(process.cwd(), [pkgJsonPath], historyIndex + 1)
            ...
            .then(pkgJson => {
                ...
                const hash = pkgJsonUtils.calcHash(pkgJson, null, config.packageHash);
            ...
            .then(
                () => {resolve(oldPkgJson)},
                () => {pullOlderBundle(config, newHash, historyIndex + 1).then(resolve, reject)}
            );
    })
}
